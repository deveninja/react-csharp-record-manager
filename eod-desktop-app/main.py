...
class MailerApp:
    ...
    def _build_invoice_mode(self, parent):
        ...
        self._hours_var, _, self._rate_var, _ = self._double_entry_row(
            details_card,
            "Hours Worked:",
            "Hourly Rate:",
        )
    ...
    def _sync_date_widgets(self):
        ...
        for widget, value in [
            (getattr(self, "_invoice_date_entry", None), self._invoice_date_var.get().strip()),
            (getattr(self, "_week_start_entry", None), self._week_start_var.get().strip()),
            (getattr(self, "_week_end_entry", None), self._week_end_var.get().strip()),
        ]:
            if widget is None:
                continue
            try:
                widget.set_date(_date_value(value))
            except Exception:
                continue
